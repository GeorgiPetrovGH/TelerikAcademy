namespace ProcessingJson
{
    ...
    class Program
    {
        ...
        private static string GetHtmlString(IEnumerable<Video> videos)
        {
            ...
            htmlString.Append("<h2>Telerik academy Videos</h2>");

            foreach (var video in videos)
            {
                htmlString.AppendFormat("<div style=\"padding:5px; margin:0px; margin-bottom:10px; border-bottom:1px solid black\">" +
                                  "<h4>{2}</h4>" + 
                                  "<iframe " +
                                  "src=\"http://www.youtube.com/embed/{1}\" " +
                                  " allowfullscreen></iframe>" +
                                  "<div><a href=\"{0}\">Watch on YouTube</a></div></div>",
